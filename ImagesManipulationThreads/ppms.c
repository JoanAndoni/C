...
// Open a matrix file and copy its contents to the structure
void readMatrixFile(char * filename, matrix_t * filter)
{
    FILE * file_ptr = NULL;
    ...
    if (filter->height == filter->width)
    {
        filter->data = malloc(filter->height * filter->width * sizeof(float));

        for (int row = 0; row < filter->height; row++)
        {
            for ( int column = 0; column < filter->width; column++)
            {
                fscanf(file_ptr, "%f", &filter->data[column + (row * filter->height)]);
            }
        }
    } else {
        printf("The file of the matrix is not square\n");
    }
...
void applyMatrixFilter(ppm_t * image, matrix_t * filter)
{
    // Local variable for an image structure
    ppm_t destination = {"", 0, 0, 0, NULL};

    //Variable that will get the elapsed time
    double elapsed;

    // Variables to mesure the time when theres no threads
    //struct timespec start, finish;

    // Start the clock for the measure without threads
    //clock_gettime(CLOCK_MONOTONIC, &start);

    // Start the clock for the measure with threads
    double start_time = omp_get_wtime();

    // Copy the header data
    strncpy (destination.magic_number, image->magic_number, 3);
    destination.max_value = image->max_value;

    //Local variable forusing the jump inside the array for the undefined pixels
    int jump = ((filter->height-1)/2);

    // Copy the size of the image
    destination.height = image->width;
    ...
    float double_matrix[filter->height][filter->width];

    for (int row = 0; row < filter->height; row++)
    {
        for ( int column = 0; column < filter->width; column++)
        {
            double_matrix[row][column] = filter->data[column + (row * filter->height)];
        }
    }

    //Local variables to help the sums of the function
    float sumMultiplicationMatrix = 0;
    float sumCoeficientsKernel = 0;
    float finalPixelValue = 0;

    //Make the openMP parallel line for the loop
    #pragma omp parallel for collapse(2) private(sumMultiplicationMatrix, sumCoeficientsKernel, finalPixelValue)

    // Apply the matrix filter
    for (int row = jump; row < image->height - jump; row++)
    {
        for (int column = jump; column < image->width - jump; column++)
        {
            for (int color = 0; color < 3; color++)
            {
                // Variable that will restart with the value of the operation
                sumMultiplicationMatrix = 0;
                sumCoeficientsKernel = 0;
                finalPixelValue = 0;

                for (int rowMatrix = -jump; rowMatrix <= jump; rowMatrix++)
                {
                    for (int colMatrix = -jump; colMatrix <= jump; colMatrix++)
                    {
                        sumMultiplicationMatrix += (double_matrix[rowMatrix+jump][colMatrix+jump]*image->pixels[row+rowMatrix][column+colMatrix].data[color]);
                        sumCoeficientsKernel += double_matrix[rowMatrix+jump][colMatrix+jump];
                    }
                }
                finalPixelValue = sumMultiplicationMatrix / sumCoeficientsKernel;
                if (finalPixelValue < 0) {
                    finalPixelValue = 0;
                } else if (finalPixelValue > 255) {
                    finalPixelValue = 255;
                }
                destination.pixels[row][column].data[color] = finalPixelValue;
            }
        }
    }

    //Stop the clock for the measure without threads
    /*
    ...
    */
    ...
    // Stop the clock and measure the elapsed time with threads
    elapsed = omp_get_wtime() - start_time;

    printf("Time elapsed: %lf\n", elapsed);

    // Time elapsed without threads:  0.006790
    // Time elapsed with threads:     0.004212
